...
{
    public class Todos_GeneralTests
        : IClassFixture<MyWebApplicationFactory<GeneralMockedReqHandler>>
    {
        private readonly MyWebApplicationFactory<GeneralMockedReqHandler> factory;

        public Todos_GeneralTests(MyWebApplicationFactory<GeneralMockedReqHandler> factory)
        {
        ...
        public async Task Update_EndpointsReturnSuccess(string url)
        {
            ...
            using var scope = factory.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<MyTodoContext>();
            var existingTodo = db.Todos.First(x => x.UserId == 1);  //Mocked user in use
        ...
        [Theory]
        [InlineData("/api/todos/crud/")]
        public async Task Update_Forbidden(string url)
        {
            // Arrange
            using var scope = factory.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<MyTodoContext>();
            var existingTodo = db.Todos.First(x => x.UserId != 1);  //Mocked user in use
            var putParam = CreateHttpContent(CreateTodoDto(existingTodo.Id, existingTodo.UserId));
        ...
        public async Task Delete_EndpointsReturnSuccess(string url)
        {
            ...
            using var scope = factory.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<MyTodoContext>();
            var todoId = db.Todos.First(x => x.UserId == 1).Id;  //Mocked user in use
        ...
        [Theory]
        [InlineData("/api/todos/crud/")]
        public async Task Delete_Forbidden(string url)
        {
            // Arrange
            using var scope = factory.Services.CreateScope();
            var db = scope.ServiceProvider.GetRequiredService<MyTodoContext>();
